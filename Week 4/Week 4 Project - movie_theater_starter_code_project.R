...
Theater1 <- function(){
  ...
  seats <- 80 # Number of seats that each theater hold 
  #Amount of snacks 
  food_amount <- 10000
  ...
  for (i in 1:7) {
    ...
    }else{number_of_screens <- sample(5:25, 1, replace = TRUE)}
    ...
    for (j in 1:number_of_screens) {
      ...
      if(movie_rate[movie[j]] == 'PG'){
        ...
        visitors_adult <- sample(80, 1)
        children <- 80 - visitors_adult 
...
employees <- function(managers,emp){
  ...
  format(round(Theater2_total_revenue, 2), nsmall = 2)
    ...
    barplot(week_days, names.arg = days, col = '#C6DEFF',
            main = 'Theater 1 Net Profit', xlab = 'Days', ylab = 'Net Profit $')
    barplot(theater2_week_days, names.arg = days, col = '#C6DEFF', 
            main = 'Theater 2 Net Profit', xlab = 'Days', ylab = 'Net Profit $')

    barplot(theaters_revenue, col = c('#C6DEFF'), beside = TRUE, 
            names.arg = c('Teater 1','Theater 2'), main = 'Comparison between Theater 1 and 2 net profit')
  }  
  ...
  # Make a barchart showing total revenue per day 
  {
   ...
   barplot(theaters_visitors, col = c('#C6DEFF'), beside = TRUE, 
           names.arg = c('Teater 1','Theater 2'), main = 'Comparison between Theater 1 and 2 total visitors')
  }    
